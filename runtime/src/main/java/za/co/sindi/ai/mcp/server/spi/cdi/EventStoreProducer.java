/**
 * 
 */
package za.co.sindi.ai.mcp.server.spi.cdi;

import jakarta.enterprise.context.ApplicationScoped;
import za.co.sindi.ai.mcp.server.EventStore;
import za.co.sindi.ai.mcp.server.InMemoryEventStore;
import za.co.sindi.commons.utils.Strings;

/**
 * @author Buhake Sindi
 * @since 12 May 2025
 */
public class EventStoreProducer extends CDIBean<EventStore> {

	public EventStoreProducer() {
		super.name("sindi_ai_mcp#" + Strings.uncapitalize(EventStore.class.getSimpleName()))
			 .scope(ApplicationScoped.class)
			 .beanClass(EventStore.class)
			 .types(EventStore.class)
			 .produce(e -> new InMemoryEventStore());
	}
}
